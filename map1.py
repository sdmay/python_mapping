...
def get_names_and_elev(x, y):
    z = str(y)
    # return folium.Popup(x)
    return folium.Popup(x + '-  Elev: ' + z + 'm', parse_html=True)
...
for lt, ln, el, n in zip(lat, lon, elev, mtnames):
    ...
    # pass the name elevation to a function to format them
        fgv.add_child(folium.CircleMarker(location=[lt, ln],  radius=5, popup=get_names_and_elev(n, el),  fill_color=color_range(el), color=color_range(el), fill_opacity=0.7, fill=True ))
...
# append population and volcanoes to the map
map.add_child(fgp)
map.add_child(fgv)
